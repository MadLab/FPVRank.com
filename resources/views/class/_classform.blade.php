<div class="row">
    <div class="form-group col-sm-12 col-lg-6">
        ...
        <input name="classId" type="text" class="form-control @error('classId') is-invalid @enderror" id="classId"
            placeholder="" required="" value="{{ !isset($class->classId) ? old('classId') : $class->classId }}">
        @error('classId')
    ...
    <div class="form-group col-sm-12 col-lg-6">
        ...
        <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" id="name" placeholder=""
            required="" value="{{ !isset($class->name) ? old('name') : $class->name }}">
        @error('name')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
    ...
    <div class="form-group col-sm-12 col-lg-6">
        ...
        <input name="description" type="text" class="form-control @error('description') is-invalid @enderror" id="name"
            placeholder="" required=""
            value="{{ !isset($class->description) ? old('description') : $class->description }}">
        @error('description')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
    ...
    <div class="form-group col-sm-12 col-lg-6">
        ...
        <select name="location" id="location" class="form-control @error('location') is-invalid @enderror"
            ...
            <option value="{{$key}}" @if(isset($class->location)) @if($class->location == $key) selected @endif
