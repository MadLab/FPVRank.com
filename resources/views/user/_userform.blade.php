<div class="row">
    <div class="form-group col-sm-12 col-lg-6">
        ...
        <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" id="name" placeholder=""
            required="" value="{{ !isset($user->name) ? old('name') : $user->name }}">
        @error('name')
    ...
    <div class="form-group col-sm-12 col-lg-6">
        ...
        <input name="email" type="email" class="form-control @error('email') is-invalid @enderror" id="email"
            placeholder="" required="" value="{{ !isset($user->email) ? old('email') : $user->email }}">
        @error('email')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
    ...
    <div class="form-group col-sm-12 col-lg-6">
        <label class="d-flex justify-content-between" for="password"><span>@if(Route::currentRouteName() == 'user.edit')
                **
                @endif{{ __('Password') }}</span> <a href="#password" data-toggle="password"><i
                    class="fa fa-eye fa-fw"></i> <span>Show</span></a></label>
    ...
    <div class="form-group col-sm-12 col-lg-6">
        <label class="d-flex justify-content-between" for="password-confirm"><span>@if(Route::currentRouteName() ==
                'user.edit')
                **
                @endif{{ __('Confirm Password') }}</span> <a href="#password-confirm" data-toggle="password"><i
                    class="fa fa-eye fa-fw"></i> <span>Show</span></a></label>
